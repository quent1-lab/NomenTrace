...
def test_export_lisible_par_openpyxl(client_spoc: TestClient, dossier_echange: Path) -> None:
    assert client_spoc.post("/api/export").json() == {"statut": "ok"}
...
def test_export_verrouille_passe_en_attente(
    client_spoc: TestClient, dossier_echange: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert client_spoc.post("/api/export").json() == {"statut": "en_attente"}
    assert client_spoc.get("/api/sante").json()["export_en_attente"] is True
    assert list((dossier_echange / "exports").glob("*.tmp")) == []
    monkeypatch.undo()
    assert client_spoc.post("/api/export").json() == {"statut": "ok"}
    assert client_spoc.get("/api/sante").json()["export_en_attente"] is False
...
def test_sauvegarde_au_demarrage_et_rotation(
    client_spoc: TestClient, chemin_base: Path, tmp_path: Path
) -> None:
